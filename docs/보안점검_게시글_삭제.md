...
- **시나리오**: 관리자가 로그인된 상태에서 `<img src="http://myserver.local/delete/5">` 태그가 포함된 페이지를 열람하기만 해도, 브라우저는 자동으로 해당 URL로 GET 요청을 보내 게시글 5번이 삭제됩니다.
...
### [심화 학습] Flask-WTF 적용하기

수동으로 난수를 생성하는 방식보다 더 안전하고 편리한 `Flask-WTF` 라이브러리 적용 방법은 다음 가이드를 참고하세요.

👉 **[가이드: Flask-WTF를 이용한 강력한 CSRF 보호 적용](가이드_Flask_WTF_CSRF_적용.md)**
